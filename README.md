...
#### 1. Prepare Data
Auto-detects optimal sequence length and creates test splits.
```bash
python src/training/step_01_prepare_data.py
```

#### 2. Train Model
Fine-tunes the model using LoRA.
- Automatically pushes the **merged FP16 model** to HuggingFace at the end.
- Use `--hf-token` to enable the push (or set `HF_TOKEN` env var).
```bash
python src/training/step_02_train.py --hf-token <token>
```

#### 3. Test Model (FP16)
Downloads the model from HuggingFace and runs comprehensive tests (Knowledge Recall & Stability).
- Supports testing specific revisions/commits.
```bash
...
- Supports **Recovery Mode** (download from HF if local files lost).
```bash
python src/training/step_04_export_gguf.py --hf-token <token>
```
...
## 🛠️ Configuration
 ...
 See **[docs/CONFIGURATION.md](docs/CONFIGURATION.md)** for a complete reference of:
 - Hyperparameters (Learning Rate, LoRA R, etc.)
 - CLI Arguments for all scripts
 ...
 See **[docs/testing.md](docs/testing.md)** for methodology and results interpretation.
